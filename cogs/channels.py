...
class Channels(commands.Cog):
    ...
    async def ping(self, ctx):
        embed = discord.Embed(title=f"Pong!", description=f'`{round(self.bot.latency * 1000)} ms` 🏓',color=0xCE2029)
        await ctx.send(embed=embed)

    @commands.command(
        name='avatar',
        description='Give you a better image of a user\'s profile picture.',
        usuage='<user>',
        aliases=['pfp', 'av']
    )
    async def avatar(self, ctx, *, user: discord.Member = None):
        if user == None:
            ...
            embed.set_author(name=f'Avatar of {ctx.author}')
            await ctx.send(embed=embed)
        else:
